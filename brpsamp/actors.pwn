...
hook OnScriptInit()
{
	...
	LoadActorDB();
	print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
	print(" addon |> Actor Editor by Blefonix RP (UnuAlex fork)");
	print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	return 1;
}
...
CMD:actorhelp(playerid, params[])
{
	...
	#if ADMIN_REQ == true
	// if(!IsPlayerAdmin(playerid))return SendClientMessage(playerid,c_warn, m_admin);
	#endif
	SendClientMessage(playerid, 0x207B18FF, "_______ACTOR GALORE HELP_______");
	SendClientMessage(playerid, 0xE99738FF, "/nactor - Create new actor.");
	SendClientMessage(playerid, 0x2EB023FF, "/dactor - Destroy actor.");
	SendClientMessage(playerid, 0xE99738FF, "/aactor - Change actor animation.");
	SendClientMessage(playerid, 0x2EB023FF, "/aexport - Export actors into script format.");
	SendClientMessage(playerid, 0xE99738FF, "/alabels - Show/Hide the actor labels.");
	SendClientMessage(playerid, 0x2EB023FF, "/apos - Relocate an actor at your current posiotion.");
	SendClientMessage(playerid, 0xE99738FF, "/atp - Relocate an actor at custom coordinates.");
	SendClientMessage(playerid, 0x2EB023FF, "/askin - Change actors skin.");
	SendClientMessage(playerid, 0xE99738FF, "/aedit - Move the actors with precision.");
	SendClientMessage(playerid, 0x2EB023FF, "/ainv - Toggle actors invulnerability.");
	SendClientMessage(playerid, 0xE99738FF, "/animadd - Add new animation to the library.");
	SendClientMessage(playerid, 0x2EB023FF, "/animedit - Edit the existing animations.");
...
CMD:nactor(playerid,params[])
{
	...
	if(actors == ACTOR_LIMIT-1)return SendClientMessage(playerid,c_warn,"(!) You can't create any more actors!");

	new skin;
	if(sscanf(params,"d",skin))return SendClientMessage(playerid,-1,"Type: /nactor [Skin Id]");
	if(skin < 0 || skin > 311)return SendClientMessage(playerid,c_warn,"Invalid skin id (0 - 311)");
    ...
    SendClientMessage(playerid,c_result,txt);
	return 1;
}
...
CMD:dactor(playerid, params[])
{
	...
	new actorid;
	if(sscanf(params,"d",actorid))return SendClientMessage(playerid,-1,"Type: /dactor [Actor Id]");
	if(!aInfo[actorid][valid])return SendClientMessage(playerid,c_warn,"Invalid actor id!");
	DeleteActor(actorid);
	new txt[64];
...
CMD:aanim(playerid,params[])
{
	...
	new actorid;
	if(sscanf(params,"d",actorid))return SendClientMessage(playerid,-1,"Type: /aanim [Actor Id]");
	if(!aInfo[actorid][valid])return SendClientMessage(playerid,c_warn,"Invalid actor id!");
	if(EditMode)return SendClientMessage(playerid,c_warn,"You can't change animations while editing actors!");
	animpage[playerid] = 1;
	aindex[playerid] = 1;
...
CMD:aexport(playerid, params[])
{
	...
	new path[32];
	if(sscanf(params,"s[32]",path))return SendClientMessage(playerid,-1,"Type:/aexport [File Name]");
	...
	{
		fclose(handle);
		SendClientMessage(playerid,c_warn,"(!) This file name is already being used!");
		return 1;
	}
	else
	{
...
CMD:alabels(playerid, params[])
{
	...
	if(alabels)
	{
		...
		SendClientMessage(playerid,c_result,"[A] - Actor labels are now hidden!");
		alabels = false;
	}
	else
	{
		...
		SendClientMessage(playerid,c_result,"[A] - Actor labels are now shown!");
		alabels = true;
	}
	return 1;
}
...
CMD:apos(playerid, params[])
{
    ...
    new actorid;
	if(sscanf(params,"d",actorid))return SendClientMessage(playerid,-1,"Type: /apos [Actor Id]");
	if(!aInfo[actorid][valid])return SendClientMessage(playerid,c_warn,"Invalid actor id!");
	new Float:pPos[4];
	GetPlayerPos(playerid,pPos[0],pPos[1],pPos[2]);
	GetPlayerFacingAngle(playerid,pPos[3]);
...
CMD:atp(playerid, params[])
{
	...
	if(sscanf(params,"dffff",actorid,posX,posY,posZ,Rot))return SendClientMessage(playerid,-1,"Type: /atp [Actor Id][PosX][PosY][PosZ][Rot]");
	if(!aInfo[actorid][valid])return SendClientMessage(playerid,c_warn,"Invalid actor id!");
...
CMD:askin(playerid, params[])
{
    ...
    new actorid, skin;
	if(sscanf(params,"dd",actorid,skin))return SendClientMessage(playerid,-1,"Type: /askin [Actor Id][Skin Id]");
	if(!aInfo[actorid][valid])return SendClientMessage(playerid,c_warn,"Invalid actor id!");
	if(skin < 0 || skin > 311)return SendClientMessage(playerid,c_warn,"Invalid skin id (0 - 311)");
...
CMD:aedit(playerid, params[])
{
    ...
    new actorid;
	if(sscanf(params,"d",actorid))return SendClientMessage(playerid,-1,"Type: /aedit [Actor Id]");
	if(!aInfo[actorid][valid])return SendClientMessage(playerid,c_warn,"Invalid actor id!");
	if(EditMode == false)
	{
	    EditMode = true;
	...
	else
	{
	    ...
	    SendClientMessage(playerid,c_result,"[A] - Actor edit canceled.");
	}
	return 1;
}
...
CMD:ainv(playerid, params[])
{
    ...
    new actorid;
	if(sscanf(params,"d",actorid))return SendClientMessage(playerid,-1,"Type: /ainv [Actor Id]");
	if(!aInfo[actorid][valid])return SendClientMessage(playerid,c_warn,"Invalid actor id!");
...
CMD:animadd(playerid, params[])
{
	...
	if(sscanf(params,"s[32]s[32]",animlib,animname))return SendClientMessage(playerid, -1, "Type: /animadd [anim-lib][anim-name]");
	if(strlen(animlib) > A_LENGHT)return SendClientMessage(playerid,c_warn,"(!) The 'anim-lib' string exceeds the character limit!");
	if(strlen(animname) > A_LENGHT)return SendClientMessage(playerid,c_warn,"(!) The 'anim-name' string exceeds the character limit!");

	AddAnimation(animlib, animname);

	PreloadAnimLib(playerid,animlib);

	SendClientMessage(playerid,c_result,"[A] -  You added a new animation. Use /animedit to change its parameters!");
	return 1;
}
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	{
	    if(strlen(inputtext) > A_NAME)return SendClientMessage(playerid,c_warn,"(!) The string introduced exceeds the character limit.");
	    ...
	    SendClientMessage(playerid,c_result,msg);
	...
	{
	    if(strlen(inputtext) > A_LENGHT)return SendClientMessage(playerid,c_warn,"(!) The string introduced exceeds the character limit.");
	    new id = animedit[playerid];
	...
	if(dialogid == D_ANIM_EDIT+6)//looping
	{
	    ...
	    SendClientMessage(playerid, c_result, "[A] - The 'Looping' setting got updated.");
	...
	if(dialogid == D_ANIM_EDIT+7)//lockx
	{
	    ...
	    SendClientMessage(playerid, c_result, "[A] - The 'Lock-X' setting got updated.");
	...
	if(dialogid == D_ANIM_EDIT+8)//locky
	{
	    ...
	    SendClientMessage(playerid, c_result, "[A] - The 'Lock-Y' setting got updated.");
	...
	if(dialogid == D_ANIM_EDIT+9)//freeze
	{
	    ...
	    SendClientMessage(playerid, c_result, "[A] - The 'Freeze' setting got updated.");
	...
	{
	    if(!actors_IsNumeric(inputtext))return SendClientMessage(playerid, c_warn, "(!) The value introduced is not a number!");
	    new id = animedit[playerid];
...
hook OnPlayerEditObject(playerid, playerobject, objectid, response, Float:fX, Float:fY, Float:fZ, Float:fRotX, Float:fRotY, Float:fRotZ)
{
	...
	if(response == EDIT_RESPONSE_FINAL)
	{
		...
		EditMode = false;
	    DestroyObject(aObject);
	    SendClientMessage(playerid,c_result,"[A] - Actor edit saved.");
	}
